...
function mapRules(rule: Rule | Media): string[][] | void {
  const definitelyRule = rule as Rule;
  if (definitelyRule.type === ASTTypes.rule && definitelyRule.selectors) {
    const selectorReal = definitelyRule.selectors
      ?.map(mapSelectors)
...
type CreateCacheReturns = { destination: string, size: number } | void;

async function createCache(cacheFile: string, content: string): Promise<CreateCacheReturns> {
  try {
